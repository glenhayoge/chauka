...
export default function AdminDashboardPage() {
  ...
  if (isLoading) return <p className="text-sm text-muted">Loading dashboard...</p>
  ...
  return (
    <div className="space-y-8 max-w-6xl">
      <div>
        ...
        <p className="text-sm text-muted mt-1">Overview of the entire Chauka platform.</p>
      </div>
        ...
        <div className="border border-border rounded-lg">
          ...
          <div className="divide-y divide-border">
            {data.recent_signups.length === 0 && (
              <p className="px-4 py-3 text-sm text-muted">No recent signups.</p>
            )}
            {data.recent_signups.map((user) => (
              <div key={user.id} className="px-4 py-2.5 flex items-center justify-between text-sm">
                <div>
                  ...
                  {user.first_name && (
                    <span className="text-muted ml-2">{user.first_name} {user.last_name}</span>
                  )}
                </div>
              ...
              <div key={entry.id} className="px-4 py-2.5 text-sm">
                <div className="flex items-center justify-between">
                  <span>
                    ...
                    <span className="text-muted mx-1">{entry.action}</span>
