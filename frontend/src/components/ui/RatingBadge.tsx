  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      <button
        ...
        className={`w-4 h-4 rounded-full border border-border ${disabled ? 'cursor-default' : 'cursor-pointer hover:scale-125'} transition-transform`}
      ...
      {open && (
        <div className="absolute right-0 top-6 z-10 bg-card border border-border rounded-lg shadow-lg py-1 min-w-[140px]">
          {ratings.map((r) => (
            <button
              key={r.id}
              onClick={() => {
          ...
          {ratingId !== null && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-2 w-full px-3 py-1.5 text-sm hover:bg-muted text-left text-muted-foreground"
              >
                <span
                  className="w-3 h-3 rounded-full border border-border flex-shrink-0"
